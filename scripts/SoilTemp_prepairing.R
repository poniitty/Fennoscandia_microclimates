...
d %>% mutate(T1 = ifelse(error_tomst %in% c(1,2,4), NA, T1),
  ...
  mutate(moist = ifelse(grepl("PIS1", id_code), NA, moist)) -> d
...
d <- fread("C:/datacloud/biogeoclimate/microclimate/data/logger/all_data.csv") %>% 
  filter(area %in% c("RAS")) %>% 
  select(id_code, datetime, error_tomst, T1:T3, moist)

d %>% mutate(moist = ifelse(T1 < 1, NA, moist),
             T1 = ifelse(error_tomst %in% c(1,2,4), NA, T1),
             T2 = ifelse(error_tomst %in% c(1,2), NA, T2),
             T3 = ifelse(error_tomst > 0, NA, T3),
             moist = ifelse(error_tomst %in% c(1,2), NA, moist)) -> d
...
fwrite(meta, "data/SoilTemp/SoilTemp_NO_TR_RASTI_meta.csv")
fwrite(d, "data/SoilTemp/SoilTemp_NO_TR_RASTI.csv")
...
meta %>% mutate(Plot_size = NA,
                ...
                FirstName = "Julia",
                LastName = "Kemppinen",
                Email = "julia.kemppinen@gmail.com",
                Institute = "University of Oulu",
                Other_contributors = "Miska Luoto: Juha Aalto") -> meta

# HABITAT CLASSES
unique(meta$Location_code)

for(i in unique(d$Plotcode)){
  print(i)
  
  if(meta[meta[,"Plotcode"] == i,"site"]>11225 |
     meta[meta[,"Plotcode"] == i,"site"] == 747 |
     meta[meta[,"Plotcode"] == i,"site"] == 475){
    meta[meta[,"Plotcode"] == i,"Habitat_type"] <- "5"
    meta[meta[,"Plotcode"] == i,"Habitat_sub_type"] <- "5.10."
  } else {
    meta[meta[,"Plotcode"] == i,"Habitat_type"] <- "4"
    meta[meta[,"Plotcode"] == i,"Habitat_sub_type"] <- "4.1."
  }
} 

# Habitat types

wetlands <- paste0("SAA",unlist(lapply(c(747,475,11226:11233),add_zeros)))

meta %>% 
  mutate(Habitat_type = "4",
         Habitat_sub_type = "4.2") %>% 
  mutate(Habitat_type = ifelse(Location_code %in% wetlands, "5",Habitat_type),
         Habitat_sub_type = ifelse(Location_code %in% wetlands, "5.10",Habitat_sub_type)) -> meta

meta %>% relocate(Habitat_type:Habitat_sub_type, .after = Logger_shielding) -> meta

meta %>% mutate(Location_code = paste0("FI_JK_MI", parse_number(Location_code))) %>% 
  mutate(Logger_code = paste0(Location_code, "_tomst"),
         Raw_data_identifier = Logger_code) -> meta

fwrite(meta, "data/SoilTemp/SoilTemp_FI_JK_MI_meta.csv")

d %>% mutate(Raw_data_identifier = paste0("FI_JK_MI", parse_number(Raw_data_identifier), "_tomst")) -> d
fwrite(d, "data/SoilTemp/SoilTemp_FI_JK_MI.csv")
